...
.ccdf_ref <- function(ycol, label, geom = "step", ...) {
  .ccdf_serie(ycol, label, "gold", geom, linetype = "dashed", ...)
}
...
.ccdf_layer <- function(s) {
  if (s$mapped) {
    ...
    lbl <- s$labels[1]
    if (s$geom == "step")
      geom_step(aes(y = .data[[s$ycol]], color = .env$lbl),
        linewidth = s$linewidth, linetype = s$linetype)
    else
      geom_point(aes(y = .data[[s$ycol]], color = .env$lbl),
        shape = s$shape, size = s$size)
  }
}
...
.ccdf_stack <- function(top, bottom) {
  ggdraw() +
    draw_plot(top,    0, 0.75, 0.75, 0.25) +
    draw_plot(bottom, 0, 0,    1,    0.75) +
...
.ccdf_discretize_panels <- function(Y, X, Z, method, fast, space_y, number_y,
  ...
  ref_lbl <- "Marginal on Z"
  p_bottom <- ggplot(df, aes(x = .data$y)) +
    geom_step(aes(y = .data$ccdf, color = .data$x), linewidth = 0.5) +
    geom_step(data = df_Z, aes(y = .data$ccdf, color = .env$ref_lbl),
      linewidth = 0.5, linetype = "dotted") +
    scale_color_manual(
      name   = "CCDF",
      values = stats::setNames(c(vx, .ccdf_ref_colour), c(lv, ref_lbl)),
      breaks = c(lv, ref_lbl),
      labels = c(paste0("CCDF X=", lv), ref_lbl),
      guide  = guide_legend(override.aes = list(
...
.ccdf_panels <- function(Y, X, Z = NULL, method = c("OLS", "logistic"),
  ...
  top_series <- if (x_fac)
    list(.ccdf_serie("ccdf", lv, vx[seq_len(l_X)], "step",
      keys = paste0("CCDF X=", lv), mapped = TRUE),
    .ccdf_ref("cdf", "CDF"))
  else
    list(.ccdf_serie("ccdf", "CCDF", vx[1], "point"),
      .ccdf_ref("cdf", "CDF"))
  p_top <- .ccdf_panel(df_X, top_series, y_lab)
...
#' constraint linking the the fits across thresholds, sfitted
...
#' thresholds do not carry any monotonicty garantee, and become hard to interpret graphically.
...
#' \code{\link[cowplot]{ggdraw}} canvas stacking the CCDF marginal on \code{Z}
#' (panel A) above the CCDF given \code{X} and \code{Z} (panel B).
#'
#' @export
#'
#' @examples
